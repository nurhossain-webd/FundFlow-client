...
export default function LoginPage() {
  ...
  const hasStartedSessionRedirect = useRef(false);
  const {
  ...
  useEffect(() => {
    if (
      isSessionPending ||
      !session ||
      hasStartedSessionRedirect.current ||
      formError
    ) {
      return;
    }

    hasStartedSessionRedirect.current = true;
    const intendedDestination = getCurrentPrivateDestination();

    void resolveAuthenticatedDestination(intendedDestination)
      .then((destination) => {
  ...
  }, [formError, isSessionPending, router, session]);
      ...
      <div className="mx-auto grid w-full max-w-5xl overflow-hidden rounded-3xl border border-border-subtle bg-surface shadow-[0_24px_70px_rgba(6,47,53,0.1)] lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <section className="p-6 sm:p-9 lg:p-12">
          ...
          <form onSubmit={submitLogin} className="space-y-5" noValidate>
            ...
            >
              {isSubmitting || isRedirecting || isSessionPending || session ? (
                <LoaderCircle
                  aria-hidden="true"
              ...
              {isRedirecting || isSessionPending || session
